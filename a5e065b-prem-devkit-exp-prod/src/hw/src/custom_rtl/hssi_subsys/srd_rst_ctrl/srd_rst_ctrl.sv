...
module srd_rst_ctrl #(
  ...
  output [NUM_CHANNELS -1:0]   o_user_rx_rst_n_100
    ...
    logic  [NUM_CHANNELS-1:0]   rst_ack_n_161;
    logic  [NUM_CHANNELS-1:0]   tx_rst_ack_n_161               ;
    logic  [NUM_CHANNELS-1:0]   rx_rst_ack_n_161               ;

    generate for (genvar i=0;i<NUM_CHANNELS;i++) begin : eth_reset_sync
 	 eth_f_altera_std_synchronizer_nocut sync_rst_ack_n_161_0 (
      .clk                       (i_clk_csr[i]),
      .reset_n                   (i_rst_ack_n[i]),
      .din                       (1'b1 ),
      .dout                      (rst_ack_n_161[i])
    );
	
 	 eth_f_altera_std_synchronizer_nocut sync_tx_rst_ack_n_161_0 (
      .clk                       (i_clk_csr[i]),
      .reset_n                   (i_tx_rst_ack_n[i]),
      .din                       (1'b1 ),
      .dout                      (tx_rst_ack_n_161[i])
    );

 	 eth_f_altera_std_synchronizer_nocut sync_rx_rst_ack_n_161_0 (
      .clk                       (i_clk_csr[i]),
      .reset_n                   (i_rx_rst_ack_n[i]),
      .din                       (1'b1 ),
      .dout                      (rx_rst_ack_n_161[i])
    );
    ...
    .i_clk                     ( i_clk_csr[i]),
    .i_pwrgood_rst_n           ( sync_i_sys_rst_n), //(pwrgood_rst_n),
