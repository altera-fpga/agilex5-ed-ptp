...
`ifndef SM_PTP_HSSI_CSR_PORT0_SOFT_IP_BASE
  `define SM_PTP_HSSI_CSR_PORT0_SOFT_IP_BASE 'h0030_0100
`endif
...
`ifndef SM_PTP_HSSI_CSR_PORT0_HARD_IP_EMAC
  `define SM_PTP_HSSI_CSR_PORT0_HARD_IP_EMAC 'h0035_0000
`endif
...
`ifndef SM_PTP_USER_CSR
  `define SM_PTP_USER_CSR  'h0020_0000
`endif
...
`define SM_PTP_HOST_WAIT_FOR_ALL_DESCR_WRBKS(h2d_port, h2d_ch, d2h_port, d2h_ch) \
  if (d2h_max_desc[d2h_port][d2h_ch] > h2d_max_desc[h2d_port][h2d_ch]) begin \
    `uvm_info(get_full_name(), \
              $sformatf("wait for d2h_desc_wrbk_cntr[%0d][%0d] == h2d_max_desc[%0d][%0d]-1", \
                        d2h_port, d2h_ch, h2d_port, h2d_ch), \
              UVM_DEBUG) \
    wait (d2h_desc_wrbk_cntr[d2h_port][d2h_ch] == h2d_max_desc[h2d_port][h2d_ch]-1); \
  end else if (d2h_max_desc[d2h_port][d2h_ch] <= h2d_max_desc[h2d_port][h2d_ch]) begin \
    `uvm_info(get_full_name(), \
              $sformatf("wait for d2h_desc_wrbk_cntr[%0d][%0d] == d2h_max_desc[%0d][%0d]-1", \
                        d2h_port, d2h_ch, d2h_port, d2h_ch), \
              UVM_DEBUG) \
    wait (d2h_desc_wrbk_cntr[d2h_port][d2h_ch] == d2h_max_desc[d2h_port][d2h_ch]-1); \
    // allow all h2d reads from DMA to complete \
  end \
  wait (h2d_desc_wrbk_cntr[h2d_port][h2d_ch] ==  h2d_max_desc[h2d_port][h2d_ch]-1); \
