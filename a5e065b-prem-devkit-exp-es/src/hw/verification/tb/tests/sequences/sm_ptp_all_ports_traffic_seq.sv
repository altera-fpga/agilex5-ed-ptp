...
// of DMA. For each channel, the payload length for each eth packet is random.
...
`ifndef SM_PTP_ALL_PORTS_TRAFFIC_SEQ__SV
`define SM_PTP_ALL_PORTS_TRAFFIC_SEQ__SV

class sm_ptp_all_ports_traffic_seq extends sm_ptp_basic_seq;

  `uvm_object_utils(sm_ptp_all_ports_traffic_seq)

  sm_ptp_basic_data_path_seq     seq_h;

  // ----------------------------------------------------------------------
  // ----------------------------------------------------------------------
  function new(name = "sm_ptp_all_ports_traffic_seq");
    super.new(name);
  endfunction: new
...
`endif
    ...
    fork
      begin
        ...
        `uvm_do_with(seq_h, {
                             ...
                             d2h_en[1][0] == 1;
                             d2h_en[1][1] == 1;
                             foreach (num_of_h2d_desc[i,j]) num_of_h2d_desc[i][j] == 20;
                             foreach (num_of_d2h_desc[i,j]) num_of_d2h_desc[i][j] == 20;
    ...
    poll_eth_stats();
  endtask: body
endclass: sm_ptp_all_ports_traffic_seq

`endif // SM_PTP_ALL_PORTS_TRAFFIC_SEQ__SV
