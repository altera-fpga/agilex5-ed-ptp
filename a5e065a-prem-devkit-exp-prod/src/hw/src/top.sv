...
output wire                o_ptp_pps 
    ...
    wire [NUM_CHANNELS-1:0]     o_tx_pll_locked_161            ;
    ...
    wire  [NUM_CHANNELS-1:0] [RXIGR_TS_DW-1:0]               hssi_ptp_rx_ingrts_tdata;      
	 ...
	 logic [DMA_CHANNELS-1:0] [31:0]                          dbg_tx_pkt_cntr, dbg_tx_valid_cntr /* synthesis syn_keep = "true"  */;
...
// **************************************************************************//  
  ...
  for (genvar i=0; i < NUM_CHANNELS; i++) begin : tx_pll_locked_161_sync
    eth_f_altera_std_synchronizer_nocut tx_pll_locked_161 (
        .clk        (o_clk_pll_161m[i]),
        .reset_n    (o_tx_pll_locked[i]),
        .din        (1'b1),        
        .dout       (o_tx_pll_locked_161[i])
    );
   end
  ...
  for (genvar i=0; i < NUM_CHANNELS; i++) begin : sts_rx_pcs_ready
   ...
   eth_f_altera_std_synchronizer_nocut sync_iopll_lock_125M (
	...
	eth_f_altera_std_synchronizer_nocut sync_iopll_lock_150M (
      .clk                       (clk_bdg_180_clk),
      .reset_n                   (iopll_locked_export),
      .din                       (1'b1),
      .dout                      (iopll_locked_export_180M)
    );
...
// `ifdef SM_PTP_CFG_25G
 .clk_bdg_100_clk                           (clk_bdg_180_clk          ),
 .rst_bdg_100_clk_clk                       (clk_bdg_180_clk          ),
 .rst_bdg_100_rst_reset_n                   (iopll_locked_export_180M ), 
//`else 
...
.iopll_0_clk100_clk                        (clk_bdg_100_clk          ),
.mtod_m0_clk_bridge_in_clk_clk             (clk_bdg_100_clk          ),
.iopll_0_clk180_clk                        (clk_bdg_180_clk          ),
...
wire        clk_todsync_sample_locked_25G;
...
// For 10G SYNC_MODE ="2" (i.e master_tod & slave_tod clks are same)
// pps_sampling clock & todsync_sampling formula is same (master_tod*256/375)
// So only one PLL used
...
// For 25G SYNC_MODE ="9" (i.e master_clk = 156.25Mhz, Slave clk =390.625Mhz)
// Slave sync clk = master_clk *128/63)
`ifdef SM_PTP_CFG_25G
todsync_smpclk_iopll_25G slavetod_samplingclk (
		.refclk   (i_clk_master_tod),    
		.locked   (clk_todsync_sample_locked_25G),    
		.rst      (~ptp_master_tod_rst_n),      
		.outclk_0 (slavetod_sampleclk_25G) //317.460Mhz
	);
`endif 
...
generate for(genvar i=0;i<NUM_CHANNELS;i++) begin : gen_mulit_inst
     ...
     // assign eth_user_tx_rst_n [i]  = o_user_tx_rst_n_161[i] & iopll_locked_export_161[i] & o_tx_pll_locked_161[i];
    ...
    intel_eth_gts_altera_std_synchronizer_nocut tx_todsync_sampling_locked_sync_inst (
        ...
        .reset_n    (1'b1),
		   `ifdef SM_PTP_CFG_25G
        .din        (clk_todsync_sample_locked_25G),
		  `else
...
// ********************************************************************* //
 ...
 `endif
        ...
        .i_clk_stod                 (clk_tx_tod[i]),
		  `ifdef SM_PTP_CFG_25G
		  .i_clk_todsync_sampling     (slavetod_sampleclk_25G),
		  `else
...
always @(posedge o_clk_pll_161m[0] or negedge eth_user_tx_rst_n[0]) 
  if(~eth_user_tx_rst_n[0])
    dbg_tx_pkt_cntr[0] <= '0;
  else if (dma_axi_st_tx_tvalid_i[0] && dma_axi_st_tx_tlast_i[0] && dma_axi_st_tx_tready_o[0])
    dbg_tx_pkt_cntr[0] <= dbg_tx_pkt_cntr[0] + 1'b1;

always @(posedge o_clk_pll_161m[0] or negedge eth_user_tx_rst_n[0]) 
  if(~eth_user_tx_rst_n[0])
    dbg_tx_pkt_cntr[1] <= '0;
  else if (dma_axi_st_tx_tvalid_i[1] && dma_axi_st_tx_tlast_i[1] && dma_axi_st_tx_tready_o[1])
    dbg_tx_pkt_cntr[1] <= dbg_tx_pkt_cntr[1] + 1'b1;
	 
always @(posedge o_clk_pll_161m[1] or negedge eth_user_tx_rst_n[1]) 
  if(~eth_user_tx_rst_n[1])
    dbg_tx_pkt_cntr[2] <= '0;
  else if (dma_axi_st_tx_tvalid_i[2] && dma_axi_st_tx_tlast_i[2] && dma_axi_st_tx_tready_o[2])
    dbg_tx_pkt_cntr[2] <= dbg_tx_pkt_cntr[2] + 1'b1;

always @(posedge o_clk_pll_161m[1] or negedge eth_user_tx_rst_n[1]) 
  if(~eth_user_tx_rst_n[1])
    dbg_tx_pkt_cntr[3] <= '0;
  else if (dma_axi_st_tx_tvalid_i[3] && dma_axi_st_tx_tlast_i[3] && dma_axi_st_tx_tready_o[3])
    dbg_tx_pkt_cntr[3] <= dbg_tx_pkt_cntr[3] + 1'b1;
...
always @(posedge o_clk_pll_161m[0] or negedge eth_user_tx_rst_n[0]) 
  if(~eth_user_tx_rst_n[0])
    dbg_tx_valid_cntr[0] <= '0;
  else begin
    if (dma_axi_st_tx_tvalid_i[0] && dma_axi_st_tx_tready_o[0] && dma_axi_st_tx_tlast_i[0])
     dbg_tx_valid_cntr[0] <= '0;
    else if (dma_axi_st_tx_tvalid_i[0] && dma_axi_st_tx_tready_o[0])
     dbg_tx_valid_cntr[0] <= dbg_tx_valid_cntr[0] + 1'b1;
   end
always @(posedge o_clk_pll_161m[0] or negedge eth_user_tx_rst_n[0]) 
  if(~eth_user_tx_rst_n[0])
    dbg_tx_valid_cntr[1] <= '0;
  else begin 
    if (dma_axi_st_tx_tvalid_i[1] && dma_axi_st_tx_tready_o[1] && dma_axi_st_tx_tlast_i[1])
      dbg_tx_valid_cntr[1] <= '0;
    else if (dma_axi_st_tx_tvalid_i[1] && dma_axi_st_tx_tready_o[1])
      dbg_tx_valid_cntr[1] <= dbg_tx_valid_cntr[1] + 1'b1;
    end
	 
always @(posedge o_clk_pll_161m[1] or negedge eth_user_tx_rst_n[1]) 
  if(~eth_user_tx_rst_n[1])
    dbg_tx_valid_cntr[2] <= '0;
  else begin
    if (dma_axi_st_tx_tvalid_i[2] && dma_axi_st_tx_tready_o[2] && dma_axi_st_tx_tlast_i[2])
      dbg_tx_valid_cntr[2] <= '0;
    else if (dma_axi_st_tx_tvalid_i[2] && dma_axi_st_tx_tready_o[2])
      dbg_tx_valid_cntr[2] <= dbg_tx_valid_cntr[2] + 1'b1;
   end
	 
always @(posedge o_clk_pll_161m[1] or negedge eth_user_tx_rst_n[1]) 
  if(~eth_user_tx_rst_n[1])
    dbg_tx_valid_cntr[3] <= '0;
  else begin
    if (dma_axi_st_tx_tvalid_i[3] && dma_axi_st_tx_tready_o[3] && dma_axi_st_tx_tlast_i[3])
      dbg_tx_valid_cntr[3] <= '0;
    else if (dma_axi_st_tx_tvalid_i[3] && dma_axi_st_tx_tready_o[3])
      dbg_tx_valid_cntr[3] <= dbg_tx_valid_cntr[3] + 1'b1;
   end
