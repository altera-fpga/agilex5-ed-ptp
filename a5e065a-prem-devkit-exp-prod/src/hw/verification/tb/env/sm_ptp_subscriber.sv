...
class sm_ptp_msgdma_subscriber extends uvm_scoreboard;
  ...
  function void load_descriptor_data(svt_axi_transaction tr);
    ...
    if (agent_type == `H2D_ST_AGENT) begin
      ...
      `uvm_info(get_full_name(),
                $sformatf(" The current descriptor's hw_owned bit is %0d", h2d_descr[port_num][chan_num][tr.addr][510]),
                UVM_DEBUG)
      if (h2d_descr[port_num][chan_num][tr.addr][510] == 1)
      ...
      if (d2h_descr[port_num][chan_num][tr.addr][510] == 1)
  ...
  function void load_h2d_dma_data(svt_axi_transaction tr);
    ...
    `uvm_info(get_full_name(), $sformatf("SCB:: Loading H2D[%0d][%0d] data from \n%s",
                                         port_num, chan_num, tr.sprint()), UVM_DEBUG)

    burst_size_bytes = 2**tr.burst_size;
    `uvm_info(get_full_name(),
              $sformatf("pending_h2d_bytes[%0d][%0d] %0d",
                        port_num, chan_num, pending_h2d_bytes[port_num][chan_num]),
              UVM_DEBUG)
    if ((pending_h2d_bytes[port_num][chan_num] == 0) && (h2d_pyld_addr[port_num][chan_num].size() !== 0)) begin
      pyld_addr = h2d_pyld_addr[port_num][chan_num].pop_front();
      desc_addr = h2d_desc_addr[port_num][chan_num].pop_front();
      `uvm_info(get_full_name(), $sformatf("pyld_addr %0h, tr.addr %0h",
                                           pyld_addr, tr.addr),
                UVM_DEBUG)
      if (pyld_addr !== tr.addr) begin
        skip_h2d_bytes[port_num][chan_num] = pyld_addr - tr.addr;
        `uvm_info(get_full_name(),
                  $sformatf("assuming unaligned payload addr, skipping %0d bytes for port %0d, chan %0d",
                            skip_h2d_bytes[port_num][chan_num], port_num, chan_num),
                  UVM_DEBUG)
      end
...
`endif
  ...
  virtual function void compare_data_bw_ports(
    ...
    compare_bytes_h2d_descr = cfgd_h2d_pyld_bytes[h2d_port][h2d_ch].sum();
    // compare_bytes_h2d_descr = cfgd_h2d_pyld_bytes[h2d_port][h2d_ch].sum with (item.index inside {[0:h2d_descr[h2d_port][h2d_ch].num()-1]} ? item:0);
    // compare_bytes_d2h_descr = cfgd_h2d_pyld_bytes[h2d_port][h2d_ch].sum with (item.index inside {[0:d2h_descr[d2h_port][d2h_ch].num()-1]} ? item:0);
    compare_bytes_d2h_descr = cfgd_h2d_pyld_bytes[h2d_port][h2d_ch].sum with (item.index inside {[0:cfgd_d2h_pyld_bytes[d2h_port][d2h_ch].size()-1]} ? item:0);
