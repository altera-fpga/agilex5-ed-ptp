...
// of DMA. For each channel, with fixed number of descriptors, the payload length
// for each eth packet is configured to 64B.
...
`ifndef SM_PTP_ALL_PORTS_64B_TRAFFIC_SEQ__SV
`define SM_PTP_ALL_PORTS_64B_TRAFFIC_SEQ__SV

class sm_ptp_all_ports_64B_traffic_seq extends sm_ptp_basic_seq;

  `uvm_object_utils(sm_ptp_all_ports_64B_traffic_seq)

  sm_ptp_basic_data_path_seq     seq_h;

  // ----------------------------------------------------------------------
  // ----------------------------------------------------------------------
  function new(name = "sm_ptp_all_ports_64B_traffic_seq");
    super.new(name);
  endfunction: new
...
`endif
    ...
    poll_eth_stats();
  endtask: body
endclass: sm_ptp_all_ports_64B_traffic_seq

`endif // SM_PTP_ALL_PORTS_64B_TRAFFIC_SEQ__SV
