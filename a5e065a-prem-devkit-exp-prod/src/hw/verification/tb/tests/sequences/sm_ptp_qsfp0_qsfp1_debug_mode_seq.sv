...
`ifndef SM_PTP_QSFP0_QSFP1_DEBUG_MODE_SEQ__SV
  `define SM_PTP_QSFP0_QSFP1_DEBUG_MODE_SEQ__SV

class sm_ptp_qsfp0_qsfp1_debug_mode_seq extends sm_ptp_qsfp_basic_seq;
  rand bit [`SVT_AXI_MAX_DATA_WIDTH-1:0] addr;
  `uvm_object_utils(sm_ptp_qsfp0_qsfp1_debug_mode_seq)

  // ----------------------------------------------------------------------
  // ----------------------------------------------------------------------
  function new(name = "sm_ptp_qsfp0_qsfp1_debug_mode_seq");
    super.new(name);
  endfunction: new
 // ----------------------------------------------------------------------
  // ----------------------------------------------------------------------
  task body();
   ...
   wr_data[0] = 64'h0000_0000_0000_0004; // QSFP selected
    ...
    repeat(1) begin
     ...
     axi_master_read(.address(`SM_PTP_QSFP1_SYSTEM_OFFSET + 'h44),
                     .burst_sz(svt_axi_transaction::BURST_SIZE_32BIT),
                     .data(rd_data), .burst_length(1));
     foreach (rd_data[r])
       `uvm_info(get_full_name(),
                 $sformatf("data read from QSFP1 FIFO register is rdata[%0d] %0h", r, rd_data[r]),
                 UVM_LOW)
    end
    #1us;
  endtask: body
endclass: sm_ptp_qsfp0_qsfp1_debug_mode_seq
