...
// For each channel, the number of desrciptors and payload length for each
// eth packet is random
...
`ifndef SM_PTP_H2D0_PATH_SEQ__SV
`define SM_PTP_H2D0_PATH_SEQ__SV

class sm_ptp_h2d0_path_seq extends sm_ptp_basic_seq;

  `uvm_object_utils(sm_ptp_h2d0_path_seq)

  sm_ptp_basic_data_path_seq     seq_h;

  // ----------------------------------------------------------------------
  // ----------------------------------------------------------------------
  function new(name = "sm_ptp_h2d0_path_seq");
    super.new(name);
  endfunction: new
  ...
  task body();
    ...
    `uvm_info(get_full_name(), "config tcam rules for dma port", UVM_LOW)
    key[0] = 'hdddd_dddd;
    key[1] = 'haaaa_dddd;
    key[2] = 'haaaa_aaaa;
...
`endif
  ...
  endtask: body
endclass: sm_ptp_h2d0_path_seq

`endif // SM_PTP_H2D0_PATH_SEQ__SV
