...
// dma port port 1 and user client 1.
// This sequence enables prefetcher for all channels of DMA port 1. For each channel,
// the number of descriptors and payload length for each eth packet is random.
// User client 1 is configured to run random number of ethernet packets
...
// packet for each of the port channels and user client port

`ifndef SM_PTP_H2D1_USER1_PATH_SEQ__SV
`define SM_PTP_H2D1_USER1_PATH_SEQ__SV

class sm_ptp_h2d1_user1_path_seq extends sm_ptp_basic_seq;

  `uvm_object_utils(sm_ptp_h2d1_user1_path_seq)

  sm_ptp_basic_data_path_seq     seq_h;

  // ----------------------------------------------------------------------
  // ----------------------------------------------------------------------
  function new(name = "sm_ptp_h2d1_user1_path_seq");
    super.new(name);
  endfunction: new
...
`endif
  endtask: body
endclass: sm_ptp_h2d1_user1_path_seq

`endif // SM_PTP_H2D1_USER1_PATH_SEQ__SV
