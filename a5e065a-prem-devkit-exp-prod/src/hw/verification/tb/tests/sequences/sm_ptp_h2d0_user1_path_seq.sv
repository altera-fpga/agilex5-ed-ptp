...
// dma port port 0 and user client 1.
// This sequence enables prefetcher for all channels of DMA port 0. For each channel,
...
`ifndef SM_PTP_H2D0_USER1_PATH_SEQ__SV
`define SM_PTP_H2D0_USER1_PATH_SEQ__SV

class sm_ptp_h2d0_user1_path_seq extends sm_ptp_basic_seq;

  `uvm_object_utils(sm_ptp_h2d0_user1_path_seq)

  sm_ptp_basic_data_path_seq     seq_h;

  // ----------------------------------------------------------------------
  // ----------------------------------------------------------------------
  function new(name = "sm_ptp_h2d0_user1_path_seq");
    super.new(name);
  endfunction: new
...
`endif
    poll_eth_stats();
  endtask: body
endclass: sm_ptp_h2d0_user1_path_seq

`endif // SM_PTP_H2D0_USER1_PATH_SEQ__SV
