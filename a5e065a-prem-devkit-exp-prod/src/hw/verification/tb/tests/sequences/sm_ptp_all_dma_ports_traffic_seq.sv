...
// of DMA. For each channel, the number of descriptors and payload length is randomized
// Also, the packet switch are configured to route the packets to intended dma
// port. The key used is SA and DA of ether packet

`ifndef SM_PTP_ALL_DMA_PORTS_TRAFFIC_SEQ__SV
`define SM_PTP_ALL_DMA_PORTS_TRAFFIC_SEQ__SV

class sm_ptp_all_dma_ports_traffic_seq extends sm_ptp_basic_seq;


  `uvm_object_utils(sm_ptp_all_dma_ports_traffic_seq)

  sm_ptp_basic_data_path_seq     seq_h;

  // ----------------------------------------------------------------------
  // ----------------------------------------------------------------------
  function new(name = "sm_ptp_all_dma_ports_traffic_seq");
    super.new(name);
  endfunction: new
...
endclass: sm_ptp_all_dma_ports_traffic_seq

`endif // SM_PTP_ALL_DMA_PORTS_TRAFFIC_SEQ__SV
