...
//#########################################################################
// Description:
// Showcases the scenario where traffic is generated from all the ports
// simultaneously
// This sequence enables prefetcher for all channels of both ports 0 and 1
// of DMA. For each channel, with random number of descriptors, the payload length
// for each eth packet is random
// user clients are enabled to generate random number of packets
...
`ifndef SM_PTP_ALL_PORTS_DMA_DESC_POLL_EN_SEQ__SV
`define SM_PTP_ALL_PORTS_DMA_DESC_POLL_EN_SEQ__SV

class sm_ptp_all_ports_dma_desc_poll_en_seq extends sm_ptp_basic_seq;

  `uvm_object_utils(sm_ptp_all_ports_dma_desc_poll_en_seq)

  sm_ptp_basic_data_path_seq     seq_h;

  // ----------------------------------------------------------------------
  // ----------------------------------------------------------------------
  function new(name = "sm_ptp_all_ports_dma_desc_poll_en_seq");
    super.new(name);
  endfunction: new
...
`else
                             ...
                             foreach (h2d_poll_en[i,j]) h2d_poll_en[i][j] == 1;
                             foreach (d2h_poll_en[i,j]) d2h_poll_en[i][j] == 1;
...
endclass: sm_ptp_all_ports_dma_desc_poll_en_seq

`endif // SM_PTP_ALL_PORTS_DMA_DESC_POLL_EN_SEQ__SV
