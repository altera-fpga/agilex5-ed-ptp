...
class qsfp_registry_component extends uvm_component;
   ...
   virtual function update_registry(qsfp_slave_seq_item qsfp_pkt);
    ...
    `uvm_info("qsfp_registry_component",
              $sformatf("addr is %d, byte enable is %d  and write is %d and wdata is %h",
                        qsfp_pkt.address,qsfp_pkt.byteenable,qsfp_pkt.write,qsfp_pkt.writedata),
              UVM_LOW);
    if ( qsfp_pkt.address == 'h7c && qsfp_pkt.byteenable =='hf &&  qsfp_pkt.write =='h1) begin
    ...
    if ( (qsfp_pkt.address >= 'h0 && qsfp_pkt.address <= 'h7c ) || en_pg == 1'b1) begin //------Lower page00 memory
     if(qsfp_pkt.qsfp_slv_pkt_type == QSFP_SLV_WRITE)begin  //write operation
       ...
       endcase
       `uvm_info("qsfp_reg_component",
                 $sformatf("Data written to QSFP register array : Address= 'h%h Data0= 'h%h,Data1 ='h%h Dta2 ='h%h Dta3 ='h%h for qsfp_slv_pkt_type %s and byten is %h",
                           qsfp_pkt.address,qsfp_registry_lo_pg0[qsfp_pkt.address],qsfp_registry_lo_pg0[(qsfp_pkt.address)+1],qsfp_registry_lo_pg0[(qsfp_pkt.address)+2],
                           qsfp_registry_lo_pg0[(qsfp_pkt.address)+3],qsfp_pkt.qsfp_slv_pkt_type,qsfp_pkt.byteenable),
                 UVM_LOW); 
      end
    end
